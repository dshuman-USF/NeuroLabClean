...
pca12 = pcadata(:,[columns(pcadata)-1 columns(pcadata)]);			% last two columns are pca 1 and pca2
pcadata(:,[columns(pcadata)-1 columns(pcadata)]) = [];				% get rid of them for now
...
if showData
  ptitle = 'final result';
  CleanData('PlotData',out(:,1:columns(out)-2));
end
  ...
  if ~isempty(times)
    ...
    if length(times2) == length(times(:,2))
      ...
      if length(times2) == length(times(:,2))-1
        ...
        times(1:rows(times)-1,3) = times2; 
        times(rows(times),3) = tdata(rows(tdata),i);  % last point is end of spike
      else
        % beginning of tdata in a big spike
        times(:,3) = times2(2:length(times2));
...
for i=1:ch
  ...
  v=v(:,[columns(v) columns(v)-1]);       % take principal components 1 and 2
...
for i=1:ch
  ...
  [junk,k]=sort(diag(d));   % sorts according to eigenvalues
  v=v(:,k);d=d(:,k);        % rearrange v and d in this order
  v=v(:,[columns(v) columns(v)-1]);       % take principle components 1 and 2
  pc=noti*v;                % project tdata onto v
